    ...
    computed: {
        ...
        orgListDataSet() {
            if(this.$store.state.summaryOrgListA.length>0 && this.$store.state.summaryOrgListB.length>0){
                ...
                // 拼接数据
                let self = [{name: arr1[0].qText,title: arr1[0].qText,data: [Math.round((arr1[2].qNum==='NaN'?0:arr1[2].qNum)*100)+'%',Math.round((arr1[3].qNum==='NaN'?0:arr1[3].qNum)),Math.round((arr1[4].qNum==='NaN'?0:arr1[4].qNum)*100)+'%'],subData:[]}];
                let others = [];
                orgData.filter(o => {
                    let temp = {};
                    temp.name = o.label;
    ...
    methods: {
        getOrgData(data, pid) {
            ...
            for (var i = 0; i < data.length; i++) {
                if (data[i].pid == pid) {
                    var obj = { label: data[i].label, id: data[i].id, data: data[i].data, children: data[i].children};
