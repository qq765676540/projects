    ...
    <van-pull-refresh class="flex flex-column" v-model="isLoading" @refresh="onRefresh">
      ...
      <div class="flex flex-column flex-1">
        <van-collapse
          ...
          accordion
          v-if="reportList&&pageActive==0&&!reportPageOpenFlag"
        >
          <van-collapse-item
            v-for="(val,key) in reportList"
            :key="key"
            :title="val.label"
            :name="key+''"
            class
          >
            <div
              v-for="(childVal,clildKey) in reportList[key].children"
              :key="clildKey"
              :class="{report: true, active:false, flex:true, 'flex-justify-center':true, 'flex-align-center':true}"
              @click.enter="openReport(childVal.label,val.id)"
            >
              ...
              <div
                class="reportFavorite flex flex-1 flex-justify-center flex-align-center"
                @click.stop="favoriteReport(childVal.label)"
              >
                <img
                  src="./images/scTrue.png"
                  width="50%"
                  v-show="reportUrlList[reportIdList[childVal.label]]?reportUrlList[reportIdList[childVal.label]].isFavorite:false"
                />
                <img
                  src="./images/scFalse.png"
                  width="50%"
                  v-show="reportUrlList[reportIdList[childVal.label]]?!reportUrlList[reportIdList[childVal.label]].isFavorite:false"
                />
              </div>
      ...
      <!-- 我的报表页面 -->
      <div
        class="flex flex-column flex-1 flex-align-center"
        style="margin-top: 2px"
        v-if="favoriteReportList.length > 0&&pageActive==1&&!reportPageOpenFlag"
      >
        <div
          v-for="(childVal,clildKey) in favoriteReportList"
          :key="clildKey"
          :class="{report: true, active:false, flex:true, 'flex-justify-center':true, 'flex-align-center':true}"
          @click.enter="openReport(childVal.label,-1)"
        >
          <div class="flex flex-1 flex-justify-center flex-align-center">
            <img src="./images/reportIcon.png" />
          </div>
      ...
      <div
        class="flex flex-column flex-1 flex-align-center"
        v-if="!favoriteReportList.length > 0&&pageActive==1"
      >
        <div class="empty"></div>
...
export default {
  ...
  mounted() {
    ...
    // setTimeout(() => {
    //   for(var i in demoData[0]){
    //     this.reportList.push(demoData[0][i]);
    //   }
    // }, 1000);
    
    // setTimeout(() => {
    //   for(var i in demoData[1]){
    //     this.$set(this.reportIdList, i, demoData[1][i])
    //   }
    // }, 5000);
    // setTimeout(() => {
    //   for(var i in demoData[2]){
    //     this.$set(this.reportUrlList, i, demoData[2][i])
    //   }
    // }, 3000);
      // reportIdList: demoData[1],
      // reportUrlList: demoData[2],
  ...
  // computed: {
    ...
    //       deep: true,
    //       handler(nData, oData) {
    //           console.log('2019-07-05 18:35:59->大门日志记录:reportList changed',nData.toString());
    //           this.loadText.push(JSON.stringify(nData))
    //           return nData.splice(0, 1);
    //       }
    //   },
    // reportList() {
    ...
    //     this.reportIdList.length > 0 &&
    //     this.reportUrlList.length > 0
    //   ) {
    //     let arr = Tools.deepClone(this.reportList);
    //     if (arr[0].children.length > 0) {
    //       this.pageActive = 1;
    //       return arr[0].children;
    //     } else {
