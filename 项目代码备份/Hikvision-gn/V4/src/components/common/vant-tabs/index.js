import { use, isDef } from '@/../node_modules/vant/es/utils';
import { raf } from '@/../node_modules/vant/es/utils/raf';
import { on, off } from '@/../node_modules/vant/es/utils/event';
import { TouchMixin } from '@/../node_modules/vant/es/mixins/touch';
import { setScrollTop, getScrollTop, getElementTop, getScrollEventTarget } from '@/../node_modules/vant/es/utils/scroll';
  ...
  methods: {
    ...
    onTouchEnd: function onTouchEnd() {
      ...
      if (direction === 'horizontal' && this.offsetX >= minSwipeDistance) {
        var next;
        if (deltaX > 0) {
          next = curActive-1 < 0 ? this.tabs.length-1 : curActive-1; 
          this.setCurActive(next);
        } else if (deltaX < 0) {
          next = curActive+1 >= this.tabs.length ? 0 : curActive+1;
          this.setCurActive(next);
        }
      }
    },
