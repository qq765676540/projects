...
<div :id="id">
    <div :id="id" class="vant-collapse">
        <van-collapse v-model="activeName" accordion>
            <van-collapse-item v-for="(item,key) in data" :title="item.title" :key="key" :name="key">
                <div slot="title">
                    <div class="flex flex-row">
                        <div class="flex flex-2 flex-align-center collapse-title-top" :class="isDefaultTitle?'collapse-title':isDefaultTitle">
                ...
                <div class="flex-1" style="max-height:150px;overflow-y: scroll !important;">
                    <div v-for="(v,i) in item.subData" :key="i" class="flex flex-column flex-justify-center flex-align-center">
                        <div v-if="v.length>2" class="flex" style="width: 90%">
                            <div class="flex-3 text-left" style="padding: 2px">{{v[0]}}</div>
                            <div class="flex-1 text-left" style="white-space: nowrap;padding: 2px">{{v[1]}}</div>
                            <div class="flex-1 text-left" style="white-space: nowrap;padding: 2px">{{v[2]}}</div>
                        </div>
                        <div class="text-left" style="width: 90%;padding: 0px 2px 5px 2px" >{{v[3]}}</div>
                    </div>
                </div>
            </van-collapse-item>
        </van-collapse>
