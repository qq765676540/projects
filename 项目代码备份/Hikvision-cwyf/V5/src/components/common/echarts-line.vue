...
import lineGetOption from "../echarts-option/line-option.js";
...
export default {
    ...
    data() {
        return {
            option: toolsBean.deepClone(lineGetOption(this.name,this.data))
        };
    },
    beforeCreate() {},
    mounted() {
        ...
        data: {
            handler(nVal, oVal) {
                if (this.isDiff(nVal, oVal)) {
                    this.option=toolsBean.deepClone(lineGetOption(this.name,nVal));
